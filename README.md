...
# 서비스
## 전체 디자인
Bootstrap을 이용한 반응형 웹디자인으로 만들었습니다. 가끔 디자인을 커스텀하고 싶은 부분은 직접 css를 이용하여 디자인을 했습니다. 

## 로그인 및 회원가입

### 유효성 검사 
keyup이벤트와 **Ajax**를 이용하여 실시간으로 중복여부 확인.   
**정규식**과 **replace 함수**를 이용하여 특수문자 입력 시 바로 삭제.   

### 그러나, keyup 이벤트 때문에 한글자씩 입력마다 DB를 호출함에 따라 DB 과부화와 비용이 증가
...
### Todo리스트
게시판의 댓글과 마찬가지로 RestFul하게 만들었습니다. Todo리스트 체크시 디자인에 신경 썼습니다.
Todo리스트 등록시 '1시간 이내', '하루 이내', '하루 이상', '시간 초과'시 각각 뱃지 색이 변하도록 만들었습니다.

### 채팅방
스터디에 참여한 사람들끼리 채팅을 하는 기능입니다. 
Stomp와 SockJS 라이브러리를 이용하여 채팅서버를 만들었습니다.
연결 성공시 데이터베이스에서 이전 채팅 기록을 불러오게 했습니다.
'카카오톡'과 비슷한 느낌을 주기위해 디자인에 신경썼습니다.

## 익명 채팅방
스터디의 채팅방과 같이 Stomp를 이용한 것은 똑같지만, **아이디**가 필요없는 **익명 채팅방**을 만들었습니다.   
...
## xss 방지 필터
'네이버'의 'lucy-xss-servlet-filter'를 사용해보려고 했으나, Ajax를 이용한 통신을 할때는 xss의 방어 적용이 안되는 문제발생 했습니다.   
...
결국 스프링 공식사이트의 docs에서 HttpMessageConverter와 관련된 클래스를 찾아보았습니다.   
찾는 도중 ByteArrayHttpMessageConverter라는 클래스가 있어서 '이미지가 Byte로 변환돼서 전송되는 방법이니 이 클래스를 이용하면 가능하지 않을까.'라고 생각하여 필터에 추가해 주었니 정상적으로 전송이 되었습니다.
...
이런 저런 방법을 시도한 끝에, '객체가 생성된 후에 키 값을 갖고 오게 하면 되지 않을까?' 라는 생각을 했고,
