...
<c:forEach items="${list}" var="board">
 <div class="row board-list-body white">
  ...
  <div class="col-10 col-md-6 ellipsis title" style="text-align:left; padding-left:0"><a class="move" href='<c:out value="${board.bno}"/>'><c:out value="${board.title}"/> <c:if test="${board.replyCnt ne 0}">&nbsp;[ <c:out value="${board.replyCnt}"/> ]</c:if></a></div>
  <div class="col-6 col-md-2 ellipsis" id="nick"><a href="#"><c:out value="${board.writer}"/></a></div>
  <div class="col-3 col-md-1" id="date"><fmt:formatDate pattern="yyyy-MM-dd" value="${board.date}"/></div>
  <div class="col-3 col-md-1" id="count"><c:out value="${board.replyCnt}"/></div>
 </div>
</c:forEach>
