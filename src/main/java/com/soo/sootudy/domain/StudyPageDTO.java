...
public class StudyPageDTO {
	...
	private StudyCriteria scri;
	
	
	private int displayPageNum = 5;
	
	public StudyPageDTO(StudyCriteria scri, int total) {
		this.scri = scri;
		this.total = total; //311개라고 할때
		this.endPage = (int) (Math.ceil(scri.getPage() / (double)displayPageNum)) * displayPageNum; //1,2,3,4,5를 해도 5만 나옴 6부터 10이나옴 11부터 15나옴
		this.startPage = (this.endPage -displayPageNum) +1; //5-5를하면 0이나옴 +1을 하니까 1페이지가 나옴 10(위에서 5다음 10이되므로)-5는 5이고 +1 하면 6
		int realEnd = (int) (Math.ceil(total / (double) scri.getPerPageNum())); // 맨 끝 구함 (올림(총 갯수 311개 / 페이지당 30개)) = 11개
		...
		this.next = this.endPage * scri.getPerPageNum() < total;  // 5,10,11*30 < 311 (사실상 마지막페이지 빼고 다 가능하다)
	}
